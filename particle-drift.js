// Particle Drift background — ported from Originkit's React/WebGL component
// (https://www.originkit.dev) to plain JS. Shaders and render logic are
// unchanged; only the React lifecycle (props/useRef/useEffect) was replaced
// with a module-scope CONFIG object and a DOMContentLoaded-time init,
// matching this project's existing hero3d.js fail-safe pattern.
...
void main(){
  ...
  try {
    ...
    if (!lineProg || !dotProg) throw new Error('Shader link failed');
    ...
    function drawFrame(now) {
      ...
      if (CONFIG.density !== builtN) {
        buildNodes(CONFIG.density, cw, ch);
        builtN = CONFIG.density;
      }
      ...
      const reach = CONFIG.hover > 0 ? 180 : 0;
      const LINKD = CONFIG.linkDistance;
      const LINKW = CONFIG.linkThickness;
      const sp = CONFIG.speed / 50;
      const th = (CONFIG.direction * Math.PI) / 180;
      ...
      const cb = parseColor(CONFIG.baseColor, [0.612, 0.639, 0.686]);
      const ca = parseColor(CONFIG.accentColor, [1, 0.231, 0.231]);
      ...
      if (nCount > 0) {
        ...
        gl.uniform1f(u(dotProg, 'uDot'), CONFIG.dotSize);
    ...
    if (prefersReducedMotion) {
      ...
      function animate(now) {
        try {
          ...
          console.warn('ParticleDrift disabled:', err.message);
